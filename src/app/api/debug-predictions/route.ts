...
export async function GET(request: Request) {
  ...
  try {
    if (mode === 'recent') {
      // Dernières N jours de prédictions
      ...
      for (const p of (data || [])) {
        ...
        if (!byDay[day]) byDay[day] = { total: 0, pending: 0, completed: 0, corrupted: 0, bySource: {}, bySport: {} };
        byDay[day].total++;
        ...
        if (!validResults.includes(p.predicted_result)) byDay[day].corrupted++;
    ...
    if (mode === 'match-date-check') {
      // Vérifier les décalages created_at vs match_date
      const startDate = new Date();
      startDate.setDate(startDate.getDate() - days);
      const startISO = startDate.toISOString().split('T')[0] + 'T00:00:00Z';
    ...
    if (mode === 'today-check') {
      // Simuler ce que le bilan ferait pour aujourd'hui/hier
    ...
    return NextResponse.json({ error: 'Mode inconnu. Utilisez: recent, match-date-check, today-check' }, { status: 400 });
  } catch (e: any) {
