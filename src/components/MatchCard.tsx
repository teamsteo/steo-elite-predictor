  ...
  return (
    <Card className={`transition-all hover:shadow-lg ${compact ? 'p-2' : ''}`}>
      <CardHeader className={`pb-2 ${compact ? 'p-3' : ''}`}>
        ...
        {/* 📊 Source des cotes (si disponible) */}
        {match.oddsSource && !match.isEstimated && (
          <div className="flex items-center justify-center gap-1 mt-1">
            <span className="text-xs text-muted-foreground">
              {match.oddsSource === 'espn-draftkings' ? '📊 DraftKings' : 
               match.oddsSource === 'the-odds-api' ? '📊 Odds API' : ''}
            </span>
          </div>
        )}
      </CardHeader>
