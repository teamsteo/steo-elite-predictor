...
// Sources à surveiller (sans GitHub Store car on utilise un stockage hybride maintenant)
const API_SOURCES = {
...
/**
 * Vérifier le store (hybride Supabase/Local/GitHub)
 */
export async function checkStoreHealth(): Promise<{
  ...
  try {
    // Utiliser le PredictionStore hybride
    const { PredictionStore } = await import('./store');
    const info = await PredictionStore.getInfoAsync();
    
    return {
      healthy: true,
      predictionCount: info.total,
      lastUpdate: info.lastUpdate || 'N/A'
    };
  } catch (error: any) {
    return {
      healthy: false,
