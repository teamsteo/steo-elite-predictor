 ...
 * - GitHub (optionnel, désactivé par défaut)
 */
...
async function loadFromGitHub(): Promise<PredictionData> {
  ...
  if (!isGitHubEnabled()) {
    console.log('📊 Prediction Tracker: GitHub désactivé, utilisation du cache local');
    return cachedData || DEFAULT_PREDICTION_DATA;
  }
  ...
  if (token) {
    try {
      const res = await fetch(
        `https://api.github.com/repos/${GITHUB_CONFIG.repo}/contents/data/predictions.json?ref=${GITHUB_CONFIG.branch}`,
        { 
          headers: { 
      ...
      if (res.ok) {
        ...
        console.log('📊 Prédictions chargées depuis GitHub API');
        return cachedData!;
      }
    } catch (e) {
      console.error('Erreur chargement prédictions API GitHub:', e);
    }
  }
  ...
  try {
    const res = await fetch(
      `https://raw.githubusercontent.com/${GITHUB_CONFIG.repo}/${GITHUB_CONFIG.branch}/data/predictions.json`,
      { next: { revalidate: 30 } }
    );
    if (res.ok) {
      cachedData = await res.json();
      console.log('📊 Prédictions chargées depuis GitHub Raw');
      return cachedData!;
    }
  } catch (e) {
  ...
  console.log('📊 Prédictions: utilisation des données par défaut');
...
async function saveToGitHub(data: PredictionData): Promise<boolean> {
  ...
  if (!isGitHubEnabled()) {
    console.log('📊 Prediction Tracker: GitHub désactivé, données en cache local uniquement');
    return true;
  }
  ...
  try {
    ...
    const getRes = await fetch(
      `https://api.github.com/repos/${GITHUB_CONFIG.repo}/contents/data/predictions.json`,
      { 
        headers: { 
          Authorization: `token ${token}`, 
