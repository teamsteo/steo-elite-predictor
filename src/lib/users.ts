...
/**
 * Hash un mot de passe avec Node.js crypto (SHA-256)
...
export async function validateUser(login: string, password: string): Promise<{
  ...
  // Hasher le mot de passe avec Node.js crypto et comparer
  const hashedPassword = hashPassword(password);
  if (user.password !== hashedPassword) return { success: false, error: 'Mot de passe incorrect' };
